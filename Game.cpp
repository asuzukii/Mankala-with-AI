...
//(so that it becomes the other player's turn) and return true. "Complete" means that the player sows 
//the seeds from a hole and takes any additional turns required or completes a capture. 
//If the player gets an additional turn, you should display the board so someone looking at the screen 
...
//You can apportion to your liking the responsibility for displaying the board between this function 
//and the move function. (Note: If when this function is called, South has no beans in play, so can't 
//make the first move, sweep any beans on the North side into North's pot and act as if the game is thus 
//over.)
void Game::play()
{
...
//Return the number of beans in the indicated hole or pot of the game's board, or −1 if the hole number 
//is invalid. This function exists so that we and you can more easily test your program.
int Game::beans(Side s, int hole) const
{
	return gameBoard->Board::beans(s, hole);
}
